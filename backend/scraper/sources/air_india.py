"""
SIH26056 — Air India (AI) Airline Portal Web Scraper Adapter.

Coordinates automated web scraping of direct airline flight availability
from Air India (Tata Group) using headless Chromium via BrowserCollector:
- Extracts authentic AI flight numbers (e.g. AI-887, AI-505), departure/arrival times,
  and economy cabin quotes.
...
def describe() -> SourceCapability:
    return SourceCapability(
        ...
        compliance_note=(
            "Extracts direct airline flight quotes via Air India booking engine under "
...
class AirIndiaFareSource(BaseFareSource):
    """Air India direct airline web scraper adapter."""
    ...
    async def _search(
        ...
        **kwargs: Any,
        ...
        search_url = (
            ...
            f"?origin={origin}&destination={destination}&departureDate={date_iso}&passengers={passengers}"
        )
        logger.info(f"Air India scraping {origin}->{destination} on {departure_date}")

        try:
            acq_res, card_texts = await self.collector.fetch_page_and_cards(
                source_id=SOURCE_NAME,
                url=search_url,
                card_selectors=("div.flight-card", "div.flight-row", "div.air-india-bound"),
                wait_selector="div.flight-card, div.flight-row",
                settle_ms=3000,
            )
            ...
            return SourceResult(
                ...
                error_message=None if observations else "No valid flight cards parsed from Air India response.",
            ...
            return SourceResult.failed(
                source_name=SOURCE_NAME,
                reason=f"Air India navigation timed out: {exc}",
                error_type="timeout",
                response_time_ms=latency_ms,
            )
    ...
    def _parse_cards(
        ...
        for txt in raw_rows:
            ...
            if total_fare < 800 or total_fare > 95000:
                continue

            times = re.findall(r"(\d{1,2}:\d{2})", txt)
            dep_time = times[0] if len(times) >= 1 else "06:45"
            dep_time_band = classify_time_band(dep_time)
            stops = 0 if any(k in txt.lower() for k in ("non stop", "non-stop", "0 stop")) else 1

            flight_match = re.search(r"AI[-\s]?(\d{3,4})", txt.upper())
            if flight_match:
                flight_no = f"AI-{flight_match.group(1)}"
                is_surrogate = False
            else:
                dep_slug = re.sub(r"[^\w]", "", dep_time)
                flight_no = f"AI-{origin_code}{destination_code}-{dep_slug}"
                is_surrogate = True
            ...
            prov = DataProvenance.for_web_scrape(
                ...
                timestamp=utc_now(),
                notes={
                    "collector": "AirIndiaPortalCollector",
            ...
            obs = FareObservation(
                ...
                stops=stops,
                is_refundable=False,
                baggage_kg=20,  # Air India includes 20kg or 25kg standard check-in
...
def create(**kwargs: Any) -> AirIndiaFareSource:
    return AirIndiaFareSource(**kwargs)
