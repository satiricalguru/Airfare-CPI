"""
SIH26056 — MakeMyTrip (OTA) Live Web Scraper Adapter.

Coordinates automated, ethical web scraping of domestic flight search results
from MakeMyTrip (MakeMyTrip India Pvt Ltd) using headless Chromium via BrowserCollector:
- Extracts exact IATA flight numbers, departure/arrival times, duration, stops,
  and branded fare families (Saver/Flex).
- Decomposes statutory Indian civil aviation fees (Base Fare, GST, UDF, Convenience).
- Stores sanitized HTML/JSON DOM snapshots in ArtifactStore with cryptographic SHA-256 verification.
- Attaches DataProvenance with AcquisitionMethod.WEB_SCRAPE.
- Non-evasion circuit breaker: immediately stops on Akamai challenge.
"""
...
def describe() -> SourceCapability:
    return SourceCapability(
        ...
        compliance_note=(
            "Extracts public domestic flight search quotes via MakeMyTrip web portal under "
...
class MakeMyTripFareSource(BaseFareSource):
    """MakeMyTrip OTA web scraper adapter utilizing BrowserCollector."""
    ...
    async def _search(
        ...
        **kwargs: Any,
        ...
        search_url = (
            f"https://www.makemytrip.com/flight/search?itinerary={origin}-{destination}-{date_str}"
            f"&tripType=O&paxType=A-{passengers}_C-0_I-0&intl=false&cabinClass=E"
        )
        logger.info(f"MakeMyTrip scraping {origin}->{destination} on {departure_date}")

        try:
            acq_res, card_texts = await self.collector.fetch_page_and_cards(
                source_id=SOURCE_NAME,
                url=search_url,
                card_selectors=("div.listingCard", "div.priceSection", "div.listingCardWrap"),
                wait_selector="div.listingCard, div.priceSection",
                settle_ms=3000,
            )
            ...
            return SourceResult(
                ...
                error_message=None if observations else "No valid flight cards parsed from MakeMyTrip response.",
            ...
            latency_ms = int((time.time() - started_at) * 1000)
            logger.warning(f"MakeMyTrip challenged: {exc}")
            ...
            latency_ms = int((time.time() - started_at) * 1000)
            logger.warning(f"MakeMyTrip policy blocked: {exc}")
            return SourceResult.failed(
                source_name=SOURCE_NAME,
            ...
            latency_ms = int((time.time() - started_at) * 1000)
            logger.warning(f"MakeMyTrip timeout for {origin}->{destination}: {exc}")
            return SourceResult.failed(
                source_name=SOURCE_NAME,
                reason=f"MakeMyTrip navigation timed out: {exc}",
                error_type="timeout",
                response_time_ms=latency_ms,
            )
        except Exception as exc:
            latency_ms = int((time.time() - started_at) * 1000)
            logger.error(f"MakeMyTrip scraper error: {exc}")
            return SourceResult.failed(
                source_name=SOURCE_NAME,
    ...
    def _parse_cards(
        ...
        for txt in raw_rows:
            txt_upper = txt.upper()
            carrier_code, carrier_name = "UNKNOWN", "Unknown Carrier"
            ...
            prov = DataProvenance.for_web_scrape(
                ...
                timestamp=utc_now(),
                notes={
                    "collector": "MakeMyTripPortalCollector",
...
def create(**kwargs: Any) -> MakeMyTripFareSource:
    return MakeMyTripFareSource(**kwargs)
