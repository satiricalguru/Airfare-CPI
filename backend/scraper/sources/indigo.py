"""
SIH26056 — IndiGo (6E) Airline Portal Web Scraper Adapter.

Coordinates automated web scraping of direct airline flight availability
from IndiGo (InterGlobe Aviation Ltd) using headless Chromium via BrowserCollector:
- Extracts exact 6E flight numbers (e.g. 6E-205, 6E-5012), departure/arrival times,
...
def describe() -> SourceCapability:
    return SourceCapability(
        ...
        compliance_note=(
            "Extracts direct airline flight quotes via IndiGo booking engine under "
...
class IndigoFareSource(BaseFareSource):
    """IndiGo direct airline web scraper adapter."""
    ...
    async def _search(
        ...
        **kwargs: Any,
        ...
        logger.info(f"IndiGo scraping {origin}->{destination} on {departure_date}")

        try:
            acq_res, card_texts = await self.collector.fetch_page_and_cards(
                source_id=SOURCE_NAME,
                url=search_url,
                card_selectors=("div.flight-select-card", "div.journey-card", "div.flight-detail-wrap"),
                wait_selector="div.flight-select-card, div.journey-card",
                settle_ms=3000,
            )
            ...
            return SourceResult(
                ...
                error_message=None if observations else "No valid flight cards parsed from IndiGo response.",
            ...
            return SourceResult.failed(
                source_name=SOURCE_NAME,
                reason=f"IndiGo navigation timed out: {exc}",
                error_type="timeout",
                response_time_ms=latency_ms,
            )
    ...
    def _parse_cards(
        ...
        for txt in raw_rows:
            ...
            dep_time = times[0] if len(times) >= 1 else "07:30"
            dep_time_band = classify_time_band(dep_time)
            stops = 0 if any(k in txt.lower() for k in ("non stop", "non-stop", "0 stop")) else 1

            flight_match = re.search(r"6E[-\s]?(\d{3,4})", txt.upper())
            if flight_match:
                flight_no = f"6E-{flight_match.group(1)}"
                is_surrogate = False
            else:
                dep_slug = re.sub(r"[^\w]", "", dep_time)
                flight_no = f"6E-{origin_code}{destination_code}-{dep_slug}"
                is_surrogate = True
            ...
            prov = DataProvenance.for_web_scrape(
                ...
                timestamp=utc_now(),
                notes={
                    "collector": "IndiGoPortalCollector",
...
def create(**kwargs: Any) -> IndigoFareSource:
    return IndigoFareSource(**kwargs)
